DROP TABLE Other_Degree;
DROP TABLE Prerequisite;
DROP TABLE Student_Class CASCADE;
DROP TABLE Section_Enrolllist CASCADE;
DROP TABLE Concentration_Course;
DROP TABLE Concentration CASCADE;
DROP TABLE Undergraduate;
DROP TABLE Master;
DROP TABLE Phd CASCADE;
DROP TABLE Graduate CASCADE;
DROP TABLE Prev_Degree;
DROP TABLE Dept_Requirement CASCADE;
DROP TABLE Requirement CASCADE;
DROP TABLE Meeting CASCADE;
DROP TABLE Probation;
DROP TABLE Thesis_Committee;
DROP TABLE Section CASCADE;
DROP TABLE Class CASCADE;
DROP TABLE Course CASCADE;
DROP TABLE Student CASCADE;
DROP TABLE Faculty CASCADE;
DROP TABLE Department CASCADE;
DROP TABLE Advisor CASCADE;
DROP TABLE Faculty_Teach_Section CASCADE;
DROP TABLE Meeting CASCADE;
DROP TABLE Enrollment CASCADE;
DROP TABLE CPQG CASCADE;
DROP TABLE CPG CASCADE;
DROP FUNCTION addGrade() CASCADE;
DROP FUNCTION updateGrade() CASCADE;
DROP FUNCTION addGradeYear() CASCADE;
DROP FUNCTION updateGradeYear() CASCADE;

